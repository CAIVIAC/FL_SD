...
class DatasetSplit(Dataset):
    def __init__(self, dataset, idxs):
        ...
        # self.idxs = list(idxs)
        self.idxs = [int(i) for i in idxs] # dj
...
def test_img(net_g, datatest, args, out_layer, return_probs=False, user_idx=-1):
    net_g.eval()
    # testing
    test_loss = 0
    correct = 0
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        # breakpoint()
        if out_layer==-1:
            log_probs = net_g(data, out_layer)
        else:
            # breakpoint()
            log_probs, logit_list = net_g(data, out_layer)
            log_probs = logit_list[0]
            # if idx == 0:
            #     print('     Output logit 3 while evaluation!')
...
def test_img_local(net_g, dataset, out_layer, args, user_idx=-1, idxs=None, return_features=False):
    net_g.eval()
    # testing
    test_loss = 0
    correct = 0
    # data_loader = DataLoader(dataset, batch_size=args.bs)
    data_loader = DataLoader(DatasetSplit(dataset, idxs), batch_size=args.bs, shuffle=False,pin_memory=True)
    l = len(data_loader)
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if out_layer==-1:
                ...
                log_probs, logit_list = net_g(data, out_layer)
                log_probs = logit_list[out_layer]
...
def test_img_local_all(net_local_list, args, dataset_test, dict_users_test, return_all=False, out_layer=-1):
    acc_test_local = np.zeros(args.num_users)
    loss_test_local = np.zeros(args.num_users)
    for idx in range(args.num_users):
        # net_local = net_local_list[idx]
        net_local = net_local_list[0]
        # net_local = copy.deepcopy(net_local_list[idx])
        # net_local.eval()
        # acc_local, loss_local = test_img_local_m(net_local, dataset_test, args, user_idx=idx, idxs=dict_users_test[idx]) #147s
        # print(acc_local)
        acc_local, loss_local = test_img_local(net_local, dataset_test, out_layer, args, user_idx=idx, idxs=dict_users_test[idx]) #3.7 s
        # print(acc_local, ' -----------')
        # acc_local, loss_local = test_img(net_local, dataset_test, args)
        # print(acc_local)
 
        acc_test_local[idx] = acc_local
        loss_test_local[idx] = loss_local
    # print(acc_test_local, acc_test_local.sum())
    # print(acc_test_local.sum())
...
#     return acc_test_local.mean(), loss_test_local.mean()
    return (acc_test_local*data_ratio_local).sum(), (loss_test_local*data_ratio_local).sum(), acc_test_local.std()
